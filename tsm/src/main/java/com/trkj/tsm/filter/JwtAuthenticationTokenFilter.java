...
public class JwtAuthenticationTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        //log.debug(request.getParameter("headers"));
        ...
        if(!StringUtils.isEmpty(jwtToken)){
            ...
            if(username != null &&
                ...
                log.info("开始授权。。。。。。。。。");
               // SysUser userDetails = myUserDetailsService.loadUserByUsername(username);
                Emp    emp=myUserDetailsService.loadUserByUsername(username);
                if(jwtTokenUtil.validateToken(jwtToken,emp.getUsername())){
                    //给使用该JWT令牌的用户进行授权
                    UsernamePasswordAuthenticationToken authenticationToken
                            = new UsernamePasswordAuthenticationToken(emp,null,emp.getAuthorities());
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
        ...
        boolean flag=matcher.match("/login", request.getServletPath())||
                ...
                matcher.match("/sel**", request.getServletPath());
        log.debug("flag={}",flag);
        return flag;
    }
}
